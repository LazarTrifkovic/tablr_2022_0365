...
| **Tehnička ograničenja** | Ceo stek: Python/FastAPI (backend), React+Vite+TypeScript (tri frontenda), Docker Compose orkestracija (bez Kubernetes-a) na jednoj razvojnoj mašini (Windows). Poliglot perzistencija: PostgreSQL (orders, auth — ACID i relacije bitne) i MongoDB (menu, barkds — polustrukturirani/brzo-menjajući podaci). Sinhrona REST komunikacija servis↔servis (nema message broker-a u ovoj iteraciji — Kafka je planirana za Fazu 4/seminarski). Sistem koristi dva deljena tajna ključa iz env promenljivih: `QR_SECRET` (drže ga menu, orders, barkds i payments — HMAC potpis stola) i `JWT_SECRET` (HS256, drže ga auth servis koji izdaje token i gateway koji ga validira) — nijedan servis nema oba. Gateway je jedina spoljna ulazna tačka — `/internal/*` i `/dev/*` rute su na njemu tvrdo blokirane. |
...
- `Bar Dashboard` → `API Gateway` : `/api/auth/login`, `/api/bar/*`, `/api/orders/*`, `/api/menu/*` (dostupnost stavki, predlog alergena, mapa stolova) — sa `Authorization: Bearer <JWT>` (REST/JSON, HTTPS) + `wss://.../ws/bar/{cafe_id}` (WebSocket)
- `Admin Panel` → `API Gateway` : `/api/auth/*`, `/api/menu/*` (kategorije/stavke/QR) — sa JWT (REST/JSON, HTTPS)
...
- `OrderRoutes` → `StatusStateMachine` : proverava da li je tražena tranzicija statusa dozvoljena (poziv unutar procesa, pre pisanja u bazu) — važi za promenu statusa sa bara (`update_status`); kod gostovog otkazivanja uslov `CREATED` je ugrađen direktno u CAS upit i ne konsultuje `STATUS_FLOW`
...
|---|---|---|
| `id` | UUID (PK) | |
| `cafe_id` | String(32), indeksirano | logička referenca na `Cafe` u menu servisu (bez FK — druga baza) |
...
```
*(`kind`, `status` i status tiketa su u modelu obični `str` — dozvoljene vrednosti
validira aplikacija na ruti, nisu enum na nivou šeme baze.)*
